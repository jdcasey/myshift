...
def get_consecutive_target_shifts(
    session: Any, schedule_id: str, target_user_id: str, start_date: datetime
) -> List[Dict[str, str]]:
    ...
    for _ in range(max_days):
        ...
        resp = session.rget(
            f"/schedules/{schedule_id}/users/{target_user_id}/on_call", params=params
        )
...
def override_main(
    ...
    parser = argparse.ArgumentParser(
        description="Override PagerDuty schedule rotations for a sequence of days."
    )
    ...
    group.add_argument(
        "--user-id", help="PagerDuty user ID to assign (overrides my_user from config)"
    )
    group.add_argument(
        "--user-email", help="PagerDuty user email to assign (overrides my_user from config)"
    )
    target_group = parser.add_mutually_exclusive_group(required=False)
    target_group.add_argument(
        "--target-user-id", help="PagerDuty user ID whose shifts will be overridden"
    )
    target_group.add_argument(
        "--target-user-email", help="PagerDuty user email whose shifts will be overridden"
    )
    parser.add_argument(
        "--dry-run",
        action="store_true",
        help="Show what would be overridden, but do not make any changes",
    )
    ...
    if not (
        (parsed_args.user_id or parsed_args.user_email or config.get("my_user"))
        and (parsed_args.target_user_id or parsed_args.target_user_email)
        and parsed_args.start_date
    ):
    ...
    if not shifts:
        print(
            f"No consecutive shifts found for user {target_user_id} starting from {parsed_args.start_date}",
            file=sys.stderr,
        )
    else:
        ...
        if parsed_args.dry_run:
            ...
            for shift in shifts:
                override = {
                    "override": {"start": shift["start"], "end": shift["end"], "user_id": user_id}
                }
                print(
                    f"Creating override for {shift['start']} to {shift['end']} for user {user_id}"
                )
